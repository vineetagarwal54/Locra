...
- [X] T010 [P] Add `ConversationMessage`/`Conversation`/`Attachment` types to `src/types/models.ts` per data-model.md's canonical model, replacing `QASession`'s `turns: {question, answer}[]` pair-array entirely:
  ```ts
...
- [X] T024 Run `tests/unit/store/conversationStore.test.ts`, `tests/contract/conversation-ownership.test.ts`, and `tests/integration/background-generation.test.ts` to green against T021/T022
